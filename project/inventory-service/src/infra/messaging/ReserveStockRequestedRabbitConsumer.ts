...
const RETRY_QUEUE_10S = "inventory.reserve_stock_requested.retry.10s";

const RK_RESERVE_REQUESTED = "fulfillment.reserve-stock-requested.v1";

export class ReserveStockRequestedRabbitConsumer {
  ...
  private async assertQueuesAndBindings(): Promise<void> {
    ...
    await this.channel.bindQueue(MAIN_QUEUE, this.config.rabbitmqExchange, RK_RESERVE_REQUESTED);

    await this.channel.assertQueue(RETRY_QUEUE_10S, {
      durable: true,
      messageTtl: 10_000,
      deadLetterExchange: this.config.rabbitmqExchange,
      deadLetterRoutingKey: RK_RESERVE_REQUESTED
    });
  }
