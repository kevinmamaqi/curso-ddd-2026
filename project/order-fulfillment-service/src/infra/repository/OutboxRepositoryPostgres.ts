...
export class OutboxRepositoryPostgres {
  ...
  async initSchema(): Promise<void> {
    await this.dbClient.query(
      ...
      CREATE INDEX IF NOT EXISTS order_outbox_messages_unsent_idx ON order_outbox_messages (created_at)
  ...
  async enqueue(params: { id: string; destination: string; body: unknown }): Promise<boolean> {
    const result = await this.dbClient.query(
      `
      INSERT INTO order_outbox_messages (id, destination, body)
      VALUES ($1, $2, $3::jsonb)
  ...
  async markSent(id: string): Promise<void> {
    await this.dbClient.query(
      `UPDATE order_outbox_messages SET sent_at = now() WHERE id = $1`,
      [id]
    );
  }
}
